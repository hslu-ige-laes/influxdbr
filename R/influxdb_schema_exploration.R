...
show_series <- function(con,
  ...
  query <- ifelse(is.null(measurement),
                  "SHOW SERIES",
                  paste0("SHOW SERIES FROM ", base::dQuote(measurement)))
...
show_tag_keys <- function(con, db, measurement = NULL) {
  query <- ifelse(is.null(measurement),
                  "SHOW TAG KEYS",
									paste0("SHOW TAG KEYS FROM ", base::dQuote(measurement)))
...
show_tag_values <- function(con, db, measurement = NULL, key) {
  ...
  query <- ifelse(is.null(measurement),
                  "SHOW TAG VALUES",
                  paste0("SHOW TAG VALUES FROM ",  base::dQuote(measurement))) %>%
...
show_field_keys <- function(con, db, measurement = NULL) {
  query <- ifelse(is.null(measurement),
                  "SHOW FIELD KEYS",
                  paste0("SHOW FIELD KEYS FROM", base::dQuote(measurement)))
